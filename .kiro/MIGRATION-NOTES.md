...
| Diretório global de skills no `$HOME` da IDE anterior | Não há equivalente direto em Kiro. O script `scripts/link-skills.sh` foi reescrito para criar links em `$HOME/.local/share/skills-engineers/` por padrão. Para ativar uma skill em um workspace Kiro, copie seu `SKILL.md` para `<workspace>/.kiro/steering/<slug>.md` E ADICIONE no topo do arquivo um bloco `---\ninclusion: manual\n---` (antes do front-matter `name:`/`description:` original). Sem isso o `SKILL.md` cai no default `inclusion: always` e é injetado em toda interação. |
| Comandos slash (`/setup-matt-pocock-skills`, `/grill-me`, etc.) | A invocação por slash é nativa em Kiro para steering files com `inclusion: manual` e para steering files com `inclusion: auto`. Como os `SKILL.md` deste repo usam front-matter no estilo da IDE de origem (`name`/`description`), transformá-los em comandos `/<slug>` requer um passo manual: prefixar o `SKILL.md` com `---\ninclusion: manual\n---` antes de copiá-lo para `.kiro/steering/`. Alternativamente, o conteúdo de qualquer `SKILL.md` pode ser injetado por referência manual em chat com `#<slug>`. |
